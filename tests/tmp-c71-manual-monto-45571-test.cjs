...
(async () => {
  ...
  await page.locator('input.payment-input--amount').fill('455.71');
  ...
  if ((state.payment.centavosAhorro || 0) !== 0.71) throw new Error(`centavosAhorro esperado 0.71, recibido ${state.payment.centavosAhorro}`);
  ...
  console.error('FALLO TEST C71 MANUAL 455.71:', err && err.message ? err.message : err);
  process.exit(1);
});
