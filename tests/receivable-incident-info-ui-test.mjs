...
try {
  ...
  assert.equal(await page.getByRole('button', { name: 'Abrir expediente', exact: true }).count(), 0);
  ...
  check('cuentas por cobrar no muestra avisos ni accesos de siniestros');
  ...
  await page.getByRole('button', { name: 'Alternar cierre' }).click();
  assert.equal(await management.isEnabled(), true);
  check('cierre de cobranza conserva sus restricciones sin mezclar siniestros');
  ...
  check('gestión disponible en móvil sin avisos de siniestros ni errores del navegador');
