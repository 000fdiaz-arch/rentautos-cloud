...
assert(rules.includes("routeRentAmountForDay(payments, item, dateKey) >= item.releaseAmount"), "La salida debe comparar el minimo con la suma diaria aplicada a renta.");
assert(!rules.includes("sum + payment.amountReceived"), "El total recibido no debe contar como renta.");
assert(source.includes('getActiveRouteReviewItems(items, payments, businessDateKey, reports)'), "Los parciales deben usar toda la ruta, no solo Trabajo.");
