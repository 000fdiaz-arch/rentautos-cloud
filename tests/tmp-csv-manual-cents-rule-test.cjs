...
(async () => {
  ...
  if ((state.payment.centavosAhorro || 0) !== 0.75) throw new Error(`centavosAhorro esperado 0.75, recibido ${state.payment.centavosAhorro}`);
  if ((state.payment.advanceApplied || 0) !== 0) throw new Error(`advanceApplied esperado 0, recibido ${state.payment.advanceApplied}`);
  if ((state.payment.otherChargesApplied?.[0]?.amount || 0) !== 115) throw new Error(`otros cargos aplicados esperados 115, recibido ${state.payment.otherChargesApplied?.[0]?.amount}`);
  if ((state.client.savings || 0) !== 0.75) throw new Error(`savings esperado 0.75, recibido ${state.client.savings}`);
  ...
  console.log('OK CSV manual con centavos: solo centavos van a ahorro y otros cargos quedan en 5.');
  console.log(JSON.stringify(state, null, 2));
  await browser.close();
})().catch((err) => {
  console.error('FALLO TEST CSV MANUAL CENTS:', err && err.message ? err.message : err);
  process.exit(1);
});
