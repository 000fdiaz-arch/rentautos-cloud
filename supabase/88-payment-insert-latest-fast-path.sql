-- Rentautos: evita recorrer payments_cloud al insertar pagos nuevos.
-- Ejecutar despues de 87-fleet-status-timeout-fast-path.sql.
--
-- El pago entrante ya contiene clientId y todos los datos necesarios para
-- mantener latest_payments_by_client_cloud. Reconstruir el historial completo
-- en cada INSERT provocaba statement timeout (57014) al aplicar pagos.
...
begin
  ...
  -- Todo pago nuevo con clientId puede actualizar el cache directamente.
  -- No importa si proviene de Ruta, banco, tarjeta o registro manual.
  if tg_op = 'INSERT'
  ...
  -- UPDATE y DELETE pueden invalidar el ultimo pago. Esos casos poco
  -- frecuentes conservan la reconstruccion historica segura.
  for v_client_id in
