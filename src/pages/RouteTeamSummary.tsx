...
export default function RouteTeamSummary({ workItems, reports }: Props) {
  return <section className="route-team-summary" aria-label="Resumen del equipo">
    ...
    <div className="route-team-summary-rows">
      {(["WC", "PTY"] as const).map((team) => {
        ...
        const confirmed = reports.filter((report) => report.status === "confirmed" && report.snapshot.routeAssignment === team).length;
        return <div className="route-team-summary-row" key={team}>
          ...
          <span><strong>{confirmed}</strong><small>Confirmados</small></span>
        </div>;
      })}
    </div>
  </section>;
