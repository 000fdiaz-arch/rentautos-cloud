...
function mergeIncidents(collisions: CollisionCaseRecord[], claims: InsuranceClaimRecord[]): UnifiedIncident[] {
...
export default function UnifiedIncidentsFollowUp({ dataOwnerUserId, canViewJudicial, canViewInsurance, refreshKey, onOpen }: Props) {
  ...
  useEffect(() => {
    ...
    ]).then(([nextCollisions, nextClaims]) => {
      if (cancelled) return;
  ...
  const incidents = useMemo(() => mergeIncidents(collisions, claims), [claims, collisions]);
  const filteredIncidents = useMemo(() => {
    ...
    return incidents.filter((incident) => {
      ...
      return [incident.unit, incident.driver, incident.plate, incident.vehicleDamage, incident.nextAction]
  ...
  return (
    <section className="panel workflow-claims-panel unified-incidents-follow-up">
      ...
      <div className="unified-incidents-toolbar">
        <label className="workflow-claim-search">Buscar<input type="search" value={search} placeholder="Unidad, chofer, placa, daño o próxima acción" onChange={(event) => setSearch(event.target.value)} /></label>
      ...
      <div className="workflow-claims-list unified-incidents-list">
        ...
        {filteredIncidents.map((incident) => {
          ...
          return <article key={incident.id} className={`unified-incident-card${expanded ? " expanded" : ""}`}>
            <div className="unified-incident-row">
              <button type="button" className="unified-incident-summary" aria-expanded={expanded} onClick={() => setExpandedId(expanded ? null : incident.id)}>
                <span className="unified-incident-identity"><strong>{incident.unit || "Sin unidad"} · {incident.driver || "Sin chofer"}</strong><small>{incident.plate || "Sin placa"} · Incidente {incident.incidentDate || "sin fecha"}</small></span>
                <span className="unified-incident-stages">{incident.collision && <small>Juicio · {incident.collision.status}</small>}{incident.claim && <small>Reclamo · {incident.claim.status}</small>}</span>
