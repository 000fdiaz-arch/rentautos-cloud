...
function judicialStepLabel(tab: JudicialCaseTab): string {
  if (tab === "attendance") return "Confirmar asistencia al juicio";
  if (tab === "workshop") return "Registrar revisión de taller";
  if (tab === "balance") return "Registrar saldo de la colisión";
  if (tab === "outcome") return "Registrar resultado del juicio";
  if (tab === "insurance") return "Gestionar reclamo al seguro";
  if (tab === "follow_up") return "Registrar próxima gestión";
  if (tab === "history") return "Revisar historial";
...
export default function CollisionsPage({ clients, payments, dataOwnerUserId, readOnly = false, onClientsChange, embedded = false, syncInsuranceClaims = true, hideCreateForm = false, initialExpandedId = "", initialSearch = "", focusedCaseId = "", initialCaseTab }: Props) {
  ...
  const casesRef = useRef<CollisionCaseRecord[]>([]);
  ...
  const [newTrialTimes, setNewTrialTimes] = useState<Record<string, string>>({});
  const [rescheduleReasons, setRescheduleReasons] = useState<Record<string, string>>({});
  const [rescheduleEvidenceFiles, setRescheduleEvidenceFiles] = useState<Record<string, File | null>>({});
  ...
  const [judicialWorkspaceTabs, setJudicialWorkspaceTabs] = useState<Record<string, JudicialWorkspaceTab>>(
    focusedCaseId ? { [focusedCaseId]: workspaceTabFromCaseTab(initialCaseTab) } : {}
  );
  ...
  const [photoGallery, setPhotoGallery] = useState<{ photos: CollisionPhotoAttachment[]; index: number; title: string; onDelete?: (photo: CollisionPhotoAttachment) => Promise<boolean> } | null>(null);
  const [administrativeClosureOpenId, setAdministrativeClosureOpenId] = useState<string | null>(null);
  const [administrativeClosureReasons, setAdministrativeClosureReasons] = useState<Record<string, string>>({});
  const [administrativeReopenOpenId, setAdministrativeReopenOpenId] = useState<string | null>(null);
  const [administrativeReopenReasons, setAdministrativeReopenReasons] = useState<Record<string, string>>({});
  ...
  async function saveTrial(): Promise<void> {
    ...
    const item: CollisionCaseRecord = {
      ...
      administrativeClosureReason: "",
      administrativelyClosedAt: null,
  ...
  async function closeCaseAdministratively(item: CollisionCaseRecord): Promise<void> {
    if (readOnly || busyId || !dataOwnerUserId || isFinalStatus(item.status)) return;
    const reason = administrativeClosureReasons[item.id]?.trim() ?? "";
    if (!reason) { setMessage("Escribe la razón del cierre administrativo."); return; }
    const now = new Date().toISOString();
    setBusyId(item.id); setMessage("");
    try {
      if (item.insuranceClaim) {
        const linkedClaim = (await loadInsuranceClaims(dataOwnerUserId)).find((claim) => (
          claim.id === item.insuranceClaim?.insuranceClaimId
          || (Boolean(item.insuranceClaim?.claimNumber)
            && claim.claimNumber.trim().toLocaleUpperCase("es") === item.insuranceClaim?.claimNumber.trim().toLocaleUpperCase("es")
            && claim.unit.trim().toLocaleUpperCase("es") === item.unit.trim().toLocaleUpperCase("es")
            && claim.incidentDate === item.incidentDate)
        ));
        if (!linkedClaim || linkedClaim.status !== "Finalizado") {
          setMessage("No se puede cerrar administrativamente mientras el reclamo al seguro esté abierto.");
          return;
        }
      }
      await persistCase({
        ...
        administrativeClosureReason: reason,
        administrativelyClosedAt: now,
        administrativeClosureHistory: [...(item.administrativeClosureHistory ?? []), { action: "CERRADO", reason, occurredAt: now }],
        updatedAt: now
      }, "Caso cerrado administrativamente.");
      setAdministrativeClosureOpenId(null);
      setAdministrativeClosureReasons((current) => ({ ...current, [item.id]: "" }));
    } catch (error) {
      console.error("No se pudo cerrar administrativamente el caso.", error);
      setMessage("No se pudo guardar el cierre administrativo.");
    } finally { setBusyId(""); }
  }

  async function reopenAdministrativeCase(item: CollisionCaseRecord): Promise<void> {
    if (readOnly || busyId || !dataOwnerUserId || item.status !== "CIERRE ADMINISTRATIVO") return;
    const reason = administrativeReopenReasons[item.id]?.trim() ?? "";
    if (!reason) { setMessage("Escribe la razón para reabrir el expediente."); return; }
    const now = new Date().toISOString();
    setBusyId(item.id); setMessage("");
    try {
      await persistCase({
        ...
        administrativeClosureReason: "",
        administrativelyClosedAt: null,
        administrativeClosureHistory: [...(item.administrativeClosureHistory ?? []), { action: "REABIERTO", reason, occurredAt: now }],
        updatedAt: now
      }, "Expediente reabierto correctamente.");
      setAdministrativeReopenOpenId(null);
      setAdministrativeReopenReasons((current) => ({ ...current, [item.id]: "" }));
    } catch (error) {
      console.error("No se pudo reabrir el expediente.", error);
      setMessage("No se pudo reabrir el expediente.");
    } finally { setBusyId(""); }
  }
  ...
  async function deleteIncidentPhoto(item: CollisionCaseRecord, photo: CollisionPhotoAttachment, confirmed = false): Promise<boolean> {
    if (!dataOwnerUserId || readOnly || busyId) return false;
    if (!confirmed && !window.confirm(`¿Eliminar ${photo.name} de las fotos del siniestro?`)) return false;
    const currentItem = casesRef.current.find((current) => current.id === item.id) ?? item;
    const now = new Date().toISOString();
    const updated: CollisionCaseRecord = {
      ...currentItem,
      incidentPhotos: (currentItem.incidentPhotos ?? []).filter((current) => current.path !== photo.path),
      editHistory: [...(currentItem.editHistory ?? []), {
        editedAt: now,
  ...
  async function applyOutcome(item: CollisionCaseRecord): Promise<void> {
    ...
    try {
      ...
      if (outcome === "NUEVA FECHA") {
        ...
        const nextTime = newTrialTimes[item.id] ?? "";
        const reason = rescheduleReasons[item.id]?.trim() ?? "";
        const evidenceFile = rescheduleEvidenceFiles[item.id];
        if (!nextDate || nextDate === item.trialDate || !nextTime || !reason || !evidenceFile) throw new Error("RESCHEDULE_REQUIRED");
        ...
        await persistCase({
          ...
          placeTime: nextTime,
          status: "NUEVA FECHA",
          trialDateHistory: [...item.trialDateHistory, {
            previousDate: item.trialDate,
            newDate: nextDate,
            previousTime: item.placeTime,
            newTime: nextTime,
            reason,
            changedAt: now,
          ...
          judicialResolutionSearchDate: null,
          updatedAt: now
        }, "Nueva fecha y hora de juicio guardadas con su documento de respaldo.");
        ...
        await persistCase({
          ...
          updatedAt: now
        }, `Resultado ABSUELTO guardado. El saldo de colisión permanecerá activo hasta adjuntar la resolución judicial. Buscarla el ${resolutionSearchDate}.`);
      } else {
      ...
      setNewTrialDates((current) => ({ ...current, [item.id]: "" }));
      setNewTrialTimes((current) => ({ ...current, [item.id]: "" }));
      setRescheduleReasons((current) => ({ ...current, [item.id]: "" }));
      setRescheduleEvidenceFiles((current) => ({ ...current, [item.id]: null }));
      setExpenseInvoiceFiles((current) => ({ ...current, [item.id]: null }));
    } catch (error) {
      if (error instanceof Error && error.message === "RESCHEDULE_REQUIRED") setMessage("Indica una fecha distinta, la hora, la razón y adjunta el documento que avala la reprogramación.");
  ...
  function selectRescheduleEvidence(caseId: string, file: File | undefined): void {
    if (!file) {
      setRescheduleEvidenceFiles((current) => ({ ...current, [caseId]: null }));
      return;
    }
    if ((!file.type.startsWith("image/") && file.type !== "application/pdf") || file.size > MAX_PHOTO_SIZE) {
      setRescheduleEvidenceFiles((current) => ({ ...current, [caseId]: null }));
      setMessage("El documento de reprogramación debe ser una imagen o PDF de 10 MB o menos.");
      return;
    }
    setMessage("");
    setRescheduleEvidenceFiles((current) => ({ ...current, [caseId]: file }));
  }
  ...
  async function saveJudicialResolution(item: CollisionCaseRecord): Promise<void> {
    ...
    try {
      ...
      let updatedCase: CollisionCaseRecord = {
        ...item,
        judicialResolutionEvidence: uploadedResolution,
        judicialResolutionSearchDate: searchDate,
        updatedAt: now
      };
      let successMessage = item.judicialResolutionEvidence
        ? "Resolución judicial reemplazada correctamente."
        : "Resolución judicial guardada. Ya puedes iniciar el reclamo al seguro.";

      if (invoice && shouldReleaseCollisionBalance) {
        const clientIndex = findCaseClientIndex(item);
        ...
        try { await onClientsChange(nextClients); }
          ...
          catch (rollbackError) { console.error("No se pudo revertir la liberación del saldo de colisión.", rollbackError); }
          throw error;
        }
        setCases((current) => current.map((candidate) => candidate.id === item.id ? updatedCase : candidate));
        successMessage = collisionCredit.creditedAmount > 0
          ? `Resolución guardada. ${USD_FORMATTER.format(collisionCredit.creditedAmount)} abonados a la colisión se aplicaron a la letra y el saldo pendiente fue retirado.`
          : "Resolución guardada. El saldo pendiente de colisión fue retirado.";
        setMessage(successMessage);
      } else {
        await persistCase(updatedCase, successMessage);
      }
  ...
  async function deleteJudicialResolution(item: CollisionCaseRecord): Promise<void> {
    ...
    if (item.expenseInvoice?.creditedToRentAt) {
      setMessage("Esta resolución ya fue utilizada para liberar el saldo de colisión. Puedes reemplazarla, pero no eliminarla.");
      return;
    }
  ...
  async function viewRescheduleEvidence(attachment: CollisionPhotoAttachment): Promise<void> {
    if (attachment.mimeType.startsWith("image/")) {
      setPhotoGallery({ photos: [attachment], index: 0, title: "Documento de reprogramación" });
      return;
    }
    const previewWindow = window.open("", "_blank");
    try {
      ...
      previewWindow?.close();
      console.error("No se pudo abrir el documento de reprogramación.", error);
      setMessage("No se pudo abrir el documento de reprogramación.");
    }
  }
    ...
    <section className="insurance-workflow-page">
      ...
      {activeTab === "agenda" && <section className={`panel workflow-claims-panel${focusedCaseId ? " workflow-claims-panel--focused judicial-focused-case" : ""}`}>
        ...
        {!focusedCaseId && <div className="workflow-claim-filters">
          ...
          <label>Estado<select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as CollisionTrialStatus | "all")}><option value="all">Todos</option><option>PENDIENTE</option><option>NUEVA FECHA</option><option>ABSUELTO</option><option>CULPABLE</option><option>CIERRE ADMINISTRATIVO</option></select></label>
        ...
        {focusedCase && <div className="judicial-focused-context">
          ...
          <span className={`judicial-focused-status status-${focusedCase.status === "ABSUELTO" ? "absolved" : focusedCase.status === "CULPABLE" ? "guilty" : focusedCase.status === "CIERRE ADMINISTRATIVO" ? "administrative" : "pending"}`}><small>Estado</small><strong>{focusedCase.status}</strong></span>
        </div>}
        <div className="workflow-claims-list">
          ...
          {filteredCases.map((item) => {
            ...
            const rescheduleEvidenceFile = rescheduleEvidenceFiles[item.id] ?? null;
            ...
            const activeWorkspaceTab = judicialWorkspaceTabs[item.id] ?? workspaceTabFromCaseTab(defaultJudicialCaseTab(item, today));
            ...
            const rescheduleDocuments = item.trialDateHistory.flatMap((event) => event.evidence ? [event.evidence] : []);
            const documentCount = (item.ticketStubPhoto ? 1 : 0)
              + (item.incidentPhotos?.length ?? 0)
              + rescheduleDocuments.length
              + (item.judicialOutcomeEvidence ? 1 : 0)
              + (item.judicialResolutionEvidence ? 1 : 0)
              + (item.expenseInvoice?.attachment ? 1 : 0)
              + (item.insuranceClaim?.photos.length ?? 0);
            ...
            const summaryActionLabel = administrativelyClosed
              ...
              : resolutionPending
                ...
                : insuranceManagementPending
                  ? item.insuranceClaim ? "Continuar gestión del reclamo al seguro" : "Iniciar reclamo al seguro"
                  : item.status === "CULPABLE"
                    ? "Expediente judicial finalizado"
            ...
            const summaryActionDetail = administrativelyClosed
              ...
              : insuranceManagementPending
                ? item.insuranceClaim ? "La resolución está adjunta; el reclamo al seguro continúa en gestión." : "La resolución está adjunta y ya puedes iniciar el reclamo al seguro."
              : requiresOutcome
                ? "La fecha del juicio llegó o está vencida."
                : item.status === "CULPABLE"
                  ? "El juicio ya tiene un resultado registrado."
                  : `Estado actual: ${item.status}.`;
            const caseTabOptions: Array<[JudicialWorkspaceTab, string, string]> = [
              ["summary", "Resumen", ""],
              ["documents", "Documentos", String(documentCount)],
              ["management", "Gestión", !managementTabOptions.length ? "Bloqueada" : isFinalStatus(item.status) ? "Cerrado" : "En curso"],
            ...
            return <article key={item.id} className={`workflow-claim-card${expanded ? " expanded" : ""}`}>
              {!focusedCaseId && <div className="workflow-claim-summary">
                <button type="button" className="workflow-claim-toggle" aria-expanded={expanded} onClick={() => { setExpandedId(expanded ? null : item.id); initializeCaseDrafts(item); }}>
                  ...
                  <span className="workflow-claim-indicators"><span className={item.status === "ABSUELTO" || item.status === "CIERRE ADMINISTRATIVO" ? "complete" : item.status === "CULPABLE" || requiresOutcome ? "missing" : "pending"}>{item.status}</span>{requiresOutcome && <span className="missing">Resultado requerido</span>}<span className={item.collisionAndRun ? "collision-runaway-yes" : "collision-runaway-no"}>Fuga: {item.collisionAndRun ? "Sí" : "No"}</span></span>
              ...
              {expanded && <div className="workflow-claim-details">
                ...
                <label className="judicial-case-mobile-nav">Sección del expediente
                  <select value={activeWorkspaceTab} onChange={(event) => setJudicialWorkspaceTabs((current) => ({ ...current, [item.id]: event.target.value as JudicialWorkspaceTab }))}>
                ...
                <div className="judicial-case-tabs" role="tablist" aria-label="Secciones del expediente judicial">
                  {caseTabOptions.map(([tab, label, badge]) => (
                    <button type="button" role="tab" key={tab} id={`judicial-${tab}-tab-${item.id}`} aria-selected={activeWorkspaceTab === tab} aria-controls={`judicial-${tab}-panel-${item.id}`} className={activeWorkspaceTab === tab ? "active" : ""} onClick={() => setJudicialWorkspaceTabs((current) => ({ ...current, [item.id]: tab }))}>{label}{badge ? <span>{badge}</span> : null}</button>
                  ))}
                </div>
                ...
                {activeWorkspaceTab === "management" && <div className="judicial-management-navigation" role="tabpanel" id={`judicial-management-panel-${item.id}`} aria-labelledby={`judicial-management-tab-${item.id}`}><div><strong>Gestión del expediente</strong><span>Selecciona el paso que necesitas consultar o completar.</span></div>{managementTabOptions.length ? <div className="judicial-management-tabs" role="tablist" aria-label="Pasos de gestión">{managementTabOptions.map(([tab, label, badge]) => <button type="button" key={tab} className={activeCaseTab === tab ? "active" : ""} aria-selected={activeCaseTab === tab} onClick={() => setJudicialCaseTabs((current) => ({ ...current, [item.id]: tab }))}>{label}{badge && <span>{badge}</span>}</button>)}</div> : <p className="judicial-document-empty">Completa primero la documentación inicial desde Resumen para habilitar la gestión judicial.</p>}</div>}
                {activeWorkspaceTab === "management" && activeCaseTab === "attendance" && <div className="judicial-case-tab-panel" role="tabpanel" id={`judicial-attendance-panel-${item.id}`}>
                ...
                {activeWorkspaceTab === "management" && activeCaseTab === "workshop" && <div className="judicial-case-tab-panel" role="tabpanel" id={`judicial-workshop-panel-${item.id}`}>
                ...
                {activeWorkspaceTab === "management" && activeCaseTab === "balance" && <div className="judicial-case-tab-panel" role="tabpanel" id={`judicial-balance-panel-${item.id}`}>
                 ...
                 {item.expenseInvoice && editingBalanceId !== item.id && <div className="collision-expense-invoice"><strong>Saldo de colisión registrado</strong><span>{item.expenseInvoice.description || item.expenseInvoice.label}</span><b>{USD_FORMATTER.format(item.expenseInvoice.amount)}</b><small>{item.expenseInvoice.creditedToRentAt ? `Saldo liberado · ${USD_FORMATTER.format(item.expenseInvoice.creditedToRentAmount ?? 0)} transferidos al estado de cuenta.` : `Evaluado el ${item.expenseInvoice.evaluatedAt || item.expenseInvoice.createdAt.slice(0, 10)} · cobro activo en otros cargos.`}</small><div className="collision-expense-invoice-actions">{item.expenseInvoice.attachment && <button type="button" className="button" onClick={() => void viewExpenseInvoice(item.expenseInvoice!.attachment!)}>Ver factura adjunta</button>}<button type="button" className="button primary" onClick={() => startEditingBalance(item)} disabled={readOnly || busyId === item.id || Boolean(item.expenseInvoice.creditedToRentAt)} title={item.expenseInvoice.creditedToRentAt ? "El saldo ya fue aplicado al estado de cuenta" : "Editar saldo y factura"}>Editar saldo y factura</button></div></div>}
                ...
                {activeWorkspaceTab === "management" && activeCaseTab === "insurance" && <div className="judicial-case-tab-panel" role="tabpanel" id={`judicial-insurance-panel-${item.id}`}>
      ...
      {photoGallery && <IncidentPhotoGalleryModal photos={photoGallery.photos} initialIndex={photoGallery.index} title={photoGallery.title} resolveUrl={resolveGalleryPhotoUrl} onDelete={photoGallery.onDelete} onClose={() => setPhotoGallery(null)} />}
    </section>
  );
}
