...
const EMPTY_FORM = { accountNumber: "", groupCode: "" };
...
export default function BankRulesSettingsPanel({ bankRules, onChange }: Props) {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>): void {
    ...
    if (editingId) {
      onChange(bankRules.map((rule) => rule.id === editingId ? { ...rule, accountNumber, groupCode, active: true, updatedAt: now } : rule));
    } else {
      onChange([{ id: crypto.randomUUID(), accountNumber, groupCode, active: true, createdAt: now, updatedAt: now }, ...bankRules]);
    }
    reset();
  }
    ...
    <>
      ...
      {inactiveRules.length > 0 && (
        <section className="panel">
          ...
          <div className="table-scroll" style={{ marginTop: 10 }}><table><thead><tr><th>Numero de cuenta</th><th>Grupo</th><th>Actualizado</th><th>Acciones</th></tr></thead>
            <tbody>{inactiveRules.map((rule) => <tr key={rule.id}><td><code>{rule.accountNumber}</code></td><td>Grupo {rule.groupCode}</td><td>{new Date(rule.updatedAt).toLocaleString("es-PA")}</td><td className="actions-cell"><button type="button" className="button ghost small" onClick={() => toggle(rule.id, true)}>Reactivar</button></td></tr>)}</tbody>
          </table></div>
        </section>
      )}
    </>
