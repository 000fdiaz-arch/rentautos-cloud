...
import { loadCloudSingletonData } from "../cloudData";
import {
  buildReceivableRows,
...
export default function ReceivablesPage({
  ...
  useEffect(() => {
    ...
    const syncFromCloud = async () => {
      ...
      try {
        const data = await loadCloudSingletonData<CollectionClosuresByDate>("collection_closures_cloud", dataOwnerUserId);
        if (cancelled) return;
        setCollectionClosuresByDate(data ?? {});
      } catch {
        if (!cancelled) setCollectionClosuresByDate({});
      }
    };
    void syncFromCloud();
