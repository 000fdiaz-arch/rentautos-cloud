...
export async function exportClientsToExcel(
  ...
  xlsx.writeFile(workbook, `rentautos-clientes-${formatFileDate(now)}.xlsx`);
}
...
export async function exportReceivablesToExcel(
  ...
  xlsx.writeFile(workbook, `rentautos-cuentas-por-cobrar-${formatFileDate(now)}.xlsx`);
}
...
export async function exportReceivablesToPdf(
  ...
  const document = new JsPDF({ orientation: "landscape" });
  document.setFontSize(14);
  document.text("Rentautos - Cuentas por Cobrar", 14, 16);
  document.setFontSize(9);
  document.text(`Generado: ${now.toLocaleDateString("es-PA")}`, 14, 22);
