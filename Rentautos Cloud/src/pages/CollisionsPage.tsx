...
const LOCAL_INSURERS_KEY = "cobrapp.module4.insurers.v1";
const LOCAL_COURTS_KEY = "cobrapp.module4.courts.v1";
const LOCAL_COURT_NUMBERS_KEY = "cobrapp.module4.court_numbers.v1";
...
export default function CollisionsPage({ clients, collisions, settings, canEdit, dataOwnerUserId, onCollisionsChange, onSettingsChange }: Props) {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem(LOCAL_COURTS_KEY);
      if (!raw) return;
      const parsed = JSON.parse(raw) as unknown[];
      if (!Array.isArray(parsed)) return;
      const normalized = Array.from(
        new Set(parsed.map((item) => String(item ?? "").trim()).filter((item) => item.length > 0))
      );
      if (normalized.length > 0) setCourtOptions(normalized);
    } catch {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem(LOCAL_INSURERS_KEY);
      if (!raw) return;
      const parsed = JSON.parse(raw) as unknown[];
      if (!Array.isArray(parsed)) return;
      const normalized = Array.from(
      ...
      if (normalized.length > 0) setInsurerOptions(normalized);
    } catch {
      // ignore local parse errors
    }
  }, []);
  ...
  function persistCourtOptions(next: string[]): void {
    ...
    localStorage.setItem(LOCAL_COURTS_KEY, JSON.stringify(next));
  ...
  function persistCourtNumberOptions(next: string[]): void {
    ...
    localStorage.setItem(LOCAL_COURT_NUMBERS_KEY, JSON.stringify(next));
  }

  function persistInsurerOptions(next: string[]): void {
    setInsurerOptions(next);
    localStorage.setItem(LOCAL_INSURERS_KEY, JSON.stringify(next));
    void onSettingsChange({
      ...settings,
      insurerOptions: next
    });
